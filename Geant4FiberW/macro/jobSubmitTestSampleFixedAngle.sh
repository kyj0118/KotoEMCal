...
for theta in 0 5 10 15 20 25 30; do
  for phi in 0 30 45 60 90; do
    optionAngle="-angleOpt=fixed -theta=${theta} -phi=${phi}"

    for energy in 200 500 1000; do
      optionEnergy="-energyOpt=fixed -momentum=${energy}"
      optionPosition="-minX=-5 -maxX=5 -minY=-5 -maxY=5 -posZ=0"
      optionOutput="-outFileName=$outputPATH/electron${energy}MeV_theta${theta}_phi${phi}_position5mm_${seed_prefix0}"
      ...
      optionOutput="-outFileName=$outputPATH/electron${energy}MeV_theta${theta}_phi${phi}MeV_position0_${seed_prefix0}"
      bsub -q s ./KotoEMCal $optionEnergy $optionAngle $optionPosition $optionParticle $optionNEvents $optionSeed $optionOutput $optionOthers
    done
  done
done
...
for theta in 0 5 10 15 20 25 30; do
  for phi in 0 30 45 60 90; do
    ...
    for energy in 200 500 1000; do
      optionEnergy="-energyOpt=fixed -momentum=${energy}"
      optionPosition="-minX=-5 -maxX=5 -minY=-5 -maxY=5 -posZ=0"
      optionOutput="-outFileName=$outputPATH/positron${energy}MeV_theta${theta}_phi${phi}_position5mm_${seed_prefix0}"
      bsub -q s ./KotoEMCal $optionEnergy $optionAngle $optionPosition $optionParticle $optionNEvents $optionSeed $optionOutput $optionOthers

      optionPosition="-posX=0 -posY=0 -posZ=0"
      optionOutput="-outFileName=$outputPATH/positron${energy}MeV_theta${theta}_phi${phi}MeV_position0_${seed_prefix0}"
      bsub -q s ./KotoEMCal $optionEnergy $optionAngle $optionPosition $optionParticle $optionNEvents $optionSeed $optionOutput $optionOthers
    done
  done
done
