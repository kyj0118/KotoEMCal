...
for energy in 200 500 1000; do
  ...
  for ((seed = 1; seed <= 20; seed++)); do
    ...
    optionOutput="-outFileName=$outputPATH/electron${energy}MeV_position5mm_${seed_prefix0}"
    ...
    optionOutput="-outFileName=$outputPATH/electron${energy}MeV_position0_${seed_prefix0}"
    bsub -q s ./KotoEMCal $optionEnergy $optionAngle $optionPosition $optionParticle $optionNEvents $optionSeed $optionOutput $optionOthers
  done
done
...
for energy in 200 500 1000; do
  ...
  for ((seed = 1; seed <= 20; seed++)); do
    ...
    optionPosition="-minX=-5 -maxX=5 -minY=-5 -maxY=5 -posZ=0"
    optionOutput="-outFileName=$outputPATH/positron${energy}MeV_position5mm_${seed_prefix0}"
    bsub -q s ./KotoEMCal $optionEnergy $optionAngle $optionPosition $optionParticle $optionNEvents $optionSeed $optionOutput $optionOthers

    optionPosition="-posX=0 -posY=0 -posZ=0"
    optionOutput="-outFileName=$outputPATH/positron${energy}MeV_position0_${seed_prefix0}"
    bsub -q s ./KotoEMCal $optionEnergy $optionAngle $optionPosition $optionParticle $optionNEvents $optionSeed $optionOutput $optionOthers
  done
done
